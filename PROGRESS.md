...
>
> **② `background-image` + 图像解码绘制管线（BG-1，batch 5 前置）**——修复前 `background-image` **未注册**（声明在 filter 阶段即被丢）、`background` 简写显式丢弃 image 分量、renderer 无任何图像命令。落地：cascade 注册（initial `none`）+ 简写展开 image 分量（`Token::Url` 无引号形态与 `url()` 函数带引号形态都认，渐变函数原样透传待实现）；renderer 新增 `image` 模块（`ImageBits`：RGBA8 + 内在尺寸，**自有类型**符合解耦 ADR；PNG 解码走 tiny-skia 内置读取器——`png` 本就在依赖图内，未新增依赖）、`RenderCommand::Rect.image`、后端 Repeat pattern 平铺（绘制序 color → image → border，符合 CSS Backgrounds L3 §2）、`PaintInput.images`（**绝对 URL** → 像素，相对解析留在加载层）；chrome 新增 `images` 模块（按**各表自身 location** 解析 `url()`、`absolutize_image_urls` 就地改写声明使 key 一致、`DocumentFetcher::fetch_bytes` 复用同一 scheme 策略、上限与失败非致命），`NavigationDoc`/`WebView`/app/文件模式/`render_html_file` 全链路携带。验证：cascade 6 条、renderer 4 条命令级 + 3 条 e2e 像素（平铺覆盖盒 / 三层绘制序 / 资源缺失页面照常）、chrome 12 条模块 + **3 条离线 e2e**（`data:` 图生效、`file://` 相对路径图生效、http 页引用 `file://` 图被 scheme 策略拒绝而背景色照常）。
>
> **③ `@media` 求值重写（MQ-V）**——修复前 unknown 直接当 false（`not <未知语法>` 会错判为**真**）、`not` 只取反紧随项而非整条 query、`only` 未实现、缺运算符被静默接受、feature 只认 px。落地：按 MQ L4 §3 实现 `Tristate`（Kleene 三值，`false and unknown = false`、`true or unknown = true`、其余 unknown 吸收；`not unknown` 仍为 unknown；二值上下文收敛为 false）、`not` 修饰整条 query、`only` 透明、malformed → `not all` 且**在下一个顶层逗号处恢复**、em/rem 按初始 16px 换算、新增 `orientation`；sheet 级 `media` 属性与规则级 `@media` 共用同一求值器。验证：cascade 15 条新测试（求值矩阵含三值语义与 malformed 恢复）+ chrome 2 条 e2e 像素（`media` 属性特性查询、orientation）。
>
> **④ inline `style` 收口（IS-V）**——实测其实现**早已在**（`from_style_attr` = §6.1 准则 4、specificity `(0,0,0)`、简写/`--*`/`!important` 全通），故本轮为验证与收口：修两处与代码矛盾的陈旧注释（"(1,0,0,0)" 与"条件评估推迟"）、文档化 §5.4.5 解析入口与完整排序语义、补 4 条端到端像素（内联胜更高特异性作者规则 / 作者 `!important` 胜内联普通 / 内联 `!important` 胜作者 `!important` / 内联简写四边框全绘）。
>
> 全量：cascade **262**、layout **132**、workspace **300** 全绿；三仓库 fmt 与 clippy `-D warnings` 干净。
>
...
>
> 上一轮（2026-09-13）**W-3 第一批（WPT 选择器对齐）**：`::part(<ident>+)` / `::slotted(<compound-selector>)` / `:host(<compound-selector>)` / `:state()` / `:heading[()]` / `:has-slotted` 落地，补齐 `:lang()`/`:dir()`，落实伪元素后置规则与 real-selector-list 禁伪元素（`:not(::before)` invalid vs `:is(::before)` forgiving-valid），并修掉 css-tokenizer 的 `--`/`--0` 被切成 `Delim`（`23ec4ec`）——75.6% → 94.3%。
>
> 上一轮（2026-09-13）**M-3 batch 3（line-height + text-transform）**：`line-height` 从 `font_size * 1.2` 硬编码近似改为按 CSS Inline L3 §4.2 精确解析（`normal`/数/长度/百分比，测量与绘制共用 cascade `text_props` 单一来源），`text-transform` 端到端生效（Unicode 全尺寸映射，布局前改写）——cascade `b66d0e4` / layout `1730e94` / renderer `bdd1dae`。
>
> 上一轮（2026-09-12）**M-3 batch 2（方向性边框 + outline 端到端）**：`border-<side>` 方向性简写与 `border-width/style/color` 的 1–4 值形式此前**整条声明被丢弃**，layout 从不映射 border（边框不占空间、`box-sizing: border-box` 对边框无效），renderer `Border` 为单组值、`outline-*` 零消费方。重建为四边模型：cascade 12 条方向性长属性 + `outline` 简写 + `thin/medium/thick`→1/3/5px（`f6c05fa`）；layout 填 taffy border rect + §4.1 used width（`3e1c3a2`）；renderer 四边独立 Border + 逐边矩形填充 + `RenderCommand::Outline`（`e48cdff`）。
>
...
| **muskitty-cascade** | ✅ 完成 | CSS Cascade L5 §4.1-§4.4/§5/§6.1/§7 + Backgrounds L3 §3.1/§4（background-image 注册 / border 方向性简写）+ UI L4 §4（outline 简写）+ Inline L3 §4.2 / Text L3 §2.1/§4（line-height / text-transform / **white-space 语义单一来源**）+ Media Queries L4 §2.2/§3（**Kleene 三值求值**：not/only/unknown/em-rem/orientation/malformed 恢复）+ CS-1d（sheet 级 disabled/alternate/media 门控） | **262** 测试全绿 | v0.1.1 | muskitty-dev/muskitty-cascade (已剥离) |
| **muskitty-layout** | ✅ 完成 | CSS Display L3 §2 + Box Model L3 §2/§3（含 border 盒模型）+ Flexbox L1 §4-§8 + Inline L3 §4.2（行高测量）+ Text L3 §2.1/§4（转换后文本 + **white-space 折叠后文本测量，nowrap/pre 单行语义**） + taffy 0.12 集成 | **132** 测试全绿 | v0.1.1 | 🔗 muskitty-dev/muskitty-layout (已剥离) |
| **muskitty-renderer** | ✅ Phase 4 B-3/B-4 + 图像管线 (BG-1) | tiny-skia 后端：DOM→CSS→Layout→Render 全链路 + HTML+CSS→PNG demo + **背景图**：`ImageBits` 自有类型 PNG 解码（tiny-skia 内置读取器，无新依赖）、`Rect.image`、Repeat pattern 平铺（color→image→border 绘制序）；white-space 与 layout 同源；`Text.wrap` | 53 + 27 + 46 测试全绿 | 本地 v0.1.0 (未发布) | 主仓库内 (未剥离) |
| **muskitty-chrome** | ✅ 窗口层 + CSS 来源接入 (CS-1) + 背景图 (BG-1) | 自绘 chrome（标签栏/工具栏/地址栏）+ winit 窗口 + 地址栏导航（http/https/file）+ **样式表来源**：DOM 文档序采集（`<style>`/`<link>`）、外链抓取（scheme 策略/上限/去重）、`@import` 加载期展开、最小 UA 样式表（HTML §15，含 `pre { white-space: pre }`）、文件热重载（监视 HTML + 外链 CSS）+ **背景图子资源**：按各表 location 解析 `url()` → 绝对化 → 抓取 → PNG 解码（`data:`/`file://`/http(s)，失败非致命） | 117 lib + 3 headless + 3 images e2e + 6 probe + 11 离线 e2e + 8 UA 像素，全绿 | 本地 v0.1.0 (未发布) | 主仓库内 (未剥离) |
...
15b. ~~**M-3 batch 2（方向性边框 + outline 端到端）**~~ ✅ 已完成（2026-09-12）：本轮把 batch 1 的"统一属性"模型补成**完整的四边模型**，并补上 outline。修复前实测的三层缺口——① cascade 对 `border-<side>` 简写与 `border-width/style/color` 的 1–4 值形式**不展开**（registry 未命中 → 整条声明丢弃，`border-left: 4px solid red` 静默无效）；② `border-<side>-style` 四向未注册；③ layout `style.border` 从未赋值（边框不占空间、`box-sizing: border-box` 对边框无效，CSS Box Model L3 §2-§3 违背）；④ renderer `Border` 单组宽/色/样式且用 inset-rect stroke；⑤ `outline-*` 注册但零消费方。落地：cascade 展开 12 条方向性长属性（`border`）/3 条（`border-<side>`）/4 条（`border-width|style|color` 多值，分量类型校验）+ `outline` 简写 + `thin/medium/thick`→1/3/5px computed 归一化 + 注册 `border-<side>-style`、移除三个简写的 registry 条目（`f6c05fa`）；layout 填 taffy border rect + §4.1 used width（style none/hidden → 0，`3e1c3a2`）；renderer 四边独立 `Border{top,right,bottom,left: Option<SideBorder>}` + 逐边矩形填充（方块拼角，miter 近似已文档化）+ `parse_border_style` 覆盖 §4.2 全集（hidden 不绘制；double/groove/ridge/inset/outset 由静默丢弃改为 solid 近似）+ `currentcolor` 用元素文字色 + 新增 `RenderCommand::Outline`（子节点之后发出 → 绘制在 border box 外、后代之上；`outline-style:auto`→solid、`outline-color:auto`→文字色；`outline-offset` 未注册固定 0）（`e48cdff`）。验证：cascade 210（89+31+73+16+1）、layout 121、renderer 102（50+37+15）、chrome 94 全绿，含**全链路像素断言**（仅左边框只染左侧 6px；5px border 撑大 border box 至 30px；outline 落在 10px margin 盒外 3px）；fmt/clippy `-D warnings` 干净。M-3 剩余缺口按批次 3–6 排期（文本属性 → opacity/z-index/visibility → border-radius/背景图 → 布局消费方），总账见 [docs/plans/2026-09-12-css-completion.md](docs/plans/2026-09-12-css-completion.md)。

15c. ~~**M-3 batch 3（line-height 精确解析 + text-transform 端到端）**~~ ✅ 已完成（2026-09-13）：修复前——`line-height` 是 layout 与 renderer **两处**各自的 `font_size * 1.2` 硬编码近似（T-3 遗留），任何 `line-height` 声明对换行、盒高与行位置完全无效；`text-transform` 注册但零消费方（大写/小写/首字大写全不生效）。落地：cascade 新增 `text_props` 模块作为**测量与绘制共用的单一语义来源**——`used_line_height_px`（px 直用/`<number>` 乘自身 font-size/百分比/`normal`=1.2×font-size；负值与 NaN 非法回退 normal，`0` 合法按规范行重叠，`+inf` 钳到 2^25 以免非有限值进入 cosmic-text Metrics）与 `apply_text_transform`（Unicode 全尺寸映射 `ß`→`SS`，`capitalize` 按空白切词近似规范的分段规则；`none`/未知关键字借用原文）；`line-height: <percentage>` 在 computed value 阶段按自身 font-size 转 px，`<number>` **保持数字形态随值继承**（子元素按自身 font-size 折算，Chrome `getComputedStyle` 同语义）（cascade `b66d0e4`）；layout `NodeContext::Text` 新增 `line_height` 字段、`measure_text` 收行高参数、文本叶在**建树时**即应用转换（测量即转换后文本），并把持续膨胀的继承文本参数收敛为 `InheritedText` 结构体（font-size/family/weight + line-height/text-transform）（layout `1730e94`）；renderer `RenderCommand::Text` 新增 `line_height`，paint 经同一 cascade 函数解析行高并在生成命令时改写内容，backend `draw_text` 不再自算行高（renderer `bdd1dae`）。验证：cascade 225（101+31+73+19+1）、layout 121（含 6 项新测量用例：40px 行高使换行盒高为 40 的整数倍、`line-height:2` 在 16/32px 字号下为 32/64px、150%→24px、"转换后文本与字面量大写测出完全相同排版"的等式断言）、renderer 110（51+42+17，含全链路像素：60px 行高把末行墨迹下推 >40px 且总墨迹差 <10%、uppercase 对同串产生不同墨迹）、chrome 94 全绿；fmt/clippy `-D warnings` 干净。**验证口径教训**：文本类像素断言只用等值/不等与位置，不用"大写更宽/更高"这类字体相关量值比较（首版即因 `l` 的 ascender 高于大写而误报）；行高像素断言要求画布足够高，否则高行高的末行被裁会令墨迹计数失真。批次 3 余项（`letter-spacing`/`word-spacing` 需自建 advance 契约、`font-style: italic` 受系统字体面影响、`white-space`/`text-indent`/`direction`）排 3b/3c。
