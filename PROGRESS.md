...
> 最后更新: 2026-09-13 | **W-3（WPT 选择器套件对齐第一批）**：`css/selectors/parsing` 从 **75.6%（384/508）** 提到 **94.3%（479/508）**（失败 124 → 29）。实现 `::part(<ident>+)` / `::slotted(<compound-selector>)` / `:host(<compound-selector>)`（参数递归收紧为复合选择器）/ `:state(<custom-ident>)` / `:heading[()]` / `:has-slotted`，补齐 `:lang()`/`:dir()`，并落实夹具钉死的伪元素后置规则（仅最右复合、`::slotted` 后禁伪类、`:has` 不可后置、`:state` 仅跟 `::part`）与 real-selector-list 禁伪元素（`:not(::before)` invalid vs `:is(::before)` forgiving-valid）；顺带修掉 css-tokenizer 的 `--`/`--0` 被切成 `Delim`（§4.3.1 漏 §4.3.9 子句，`23ec4ec`）。selectors `f8d2002`。剩余 29 例：28 例 An+B 符号保真（需 tokenizer 暴露 number 符号位，单列 W-3b）+ 1 例 tentative 夹具自相矛盾（`div + div` valid 却 `div > span` invalid，已在文档记录并排除硬断言）。6 个夹具升为硬断言，新增 13 例回归测试；下游全绿（css-parser 100 / css-values 150 / cssom 104 / cascade 225 / layout 127 / html5-parser 113 / workspace 218）
>
...
| **muskitty-selectors** | ✅ 完成 | Selectors L4 §3/§4/§5/§6/§13/§14/§15/§17/§18 + L5 §state/§heading + Shadow L1（`::part`/`::slotted`/`:host()`/`:has-slotted`） | 单元全绿 + WPT selectors/parsing **94.3% (479/508)**，9 个夹具硬断言 | v0.1.0 | muskitty-dev/muskitty-selectors |
