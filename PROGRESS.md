...
> 最后更新: 2026-09-12 | **M-3 batch 2（CSS 补全：方向性边框 + outline 端到端）**：`border-<side>` 方向性简写与 `border-width/style/color` 的 1–4 值形式此前**整条声明被丢弃**（cascade 不展开 → registry 未命中），layout 从不映射 border（边框不占空间、`box-sizing: border-box` 对边框无效），renderer `Border` 为单组值、`outline-*` 零消费方。本批：cascade 展开 12 条方向性长属性 + `outline` 简写 + `thin/medium/thick`→1/3/5px 归一化（`f6c05fa`）；layout 填 taffy border rect + §4.1 used width 语义（`3e1c3a2`）；renderer 四边独立 Border + 逐边矩形填充 + 新增 `RenderCommand::Outline`（子节点之后发出、盒外绘制）（`e48cdff`）。验证：cascade 210 / layout 121 / renderer 102 / chrome 94 全绿（含全链路像素断言：仅左边框着色、border 撑大盒子、outline 在盒外 3px），fmt/clippy 干净。M-3 剩余缺口与批次排期见 [docs/plans/2026-09-12-css-completion.md](docs/plans/2026-09-12-css-completion.md)
>
> 上一轮（2026-09-07）P0 止血批核查与集成：审计 2026-09-06 的 4 项 P0（V-1 / CSS-P1 / SEL-1 / SEL-2）由架构师在 muskitty-dev 各 crate 远端落地（附 SEL-3、CAS-1/2/3），主仓库完成合并与集成验证；真机 GUI 检测因控制台锁定暂挂，另修复 rgba_to_0rgb R/B 互换（9562f0f）。
>
...
| **muskitty-cascade** | ✅ 完成 | CSS Cascade L5 §4.1-§4.4/§5/§6.1/§7 + Backgrounds L3 §4（border 方向性简写）+ UI L4 §4（outline 简写） | 210 测试全绿 (89 lib + 31 filter + 73 集成 + 16 style_tree + 1 doctest) | 本地 v0.1.0 (未发布) | muskitty-dev/muskitty-cascade (已剥离) |
| **muskitty-layout** | ✅ 完成 | CSS Display L3 §2 + Box Model L3 §2/§3（含 border 盒模型）+ Flexbox L1 §4-§8 + taffy 0.12 集成 | 121 测试全绿 | 本地 v0.1.0 (未发布) | 🔗 muskitty-dev/muskitty-layout (已剥离) |
