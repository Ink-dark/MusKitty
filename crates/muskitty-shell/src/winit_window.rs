...
//!
//! 本模块仅在 `winit-backend` feature 下编译（`--no-default-features`
//! 时 shell 仍可无窗口渲染，见 [`crate::page`]）。
...
pub fn rgba_to_0rgb(data: &[u8]) -> Vec<u32> {
...
impl WinitWindow {
    ...
    pub fn new(id: u64, window: Rc<Window>) -> Result<Self, Box<dyn std::error::Error>> {
