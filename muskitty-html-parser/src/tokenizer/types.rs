...
pub enum Token {
    ...
    /// An end tag token.
    ///
    /// Note: per the spec, attributes on end tag tokens are ignored during
    /// tree construction (parse error `end-tag-with-attributes`), but the
    /// tokenizer still emits them.
    EndTag(TagToken),
...
/// A tag token (used for both start and end tags).
///
...
/// structure, though attributes on end tags are a parse error.
#[derive(Debug, Clone, PartialEq)]
pub struct TagToken {
...
/// Tokenizer states.
///
/// WHATWG §13.2.5 defines 85+ states. This enum lists the states that
/// the initial tokenizer implementation will handle. Additional states
/// (character references, CDATA sections, processing instructions) are
